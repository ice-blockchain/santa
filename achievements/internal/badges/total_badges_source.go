...
func (b *totalBadgesSource) Process(ctx context.Context, message *messagebroker.Message) error {
	...
	if achievedBadge.Unachieved {
		return errors.Wrapf(b.updateTotalBadgesCount(achievedBadge.Name, -1),
			"totalBadgesSource: failed to update total badge count for badge:%v", achievedBadge.Name)
	}
