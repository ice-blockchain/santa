...
func (m *miningEventSourceProcessor) Process(ctx context.Context, message *messagebroker.Message) error {
	...
	if err != nil {
		return errors.Wrapf(err, "miningEventSourceProcessor: cannot handle user mining session")
	}
...
func (m *miningEventSourceProcessor) updateConsecutiveMiningSessions(userID UserID, lastStartedTS uint64) error {
	key := tarantool.StringKey{S: userID}
	ops := []tarantool.Op{
		{Op: "=", Field: 1, Arg: lastStartedTS}, // | last_mining_started_at = lastStartedTS
		{Op: "+", Field: 2, Arg: 1},             // | max_count +=1
	}
	return errors.Wrapf(m.db.UpdateTyped(consecutiveUserMiningSessionsSpace, "pk_unnamed_CONSECUTIVE_USER_MINING_SESSIONS_1", key, ops, &[]*consecutiveUserMiningSessions{}),
...
func (m *miningEventSourceProcessor) resetConsecutiveMiningSessions(userID UserID, lastStartedTS uint64) error {
	key := tarantool.StringKey{S: userID}
	ops := []tarantool.Op{
		{Op: "=", Field: 1, Arg: lastStartedTS}, // | last_mining_started_at = lastStartedTS
		{Op: "=", Field: 2, Arg: 1},             // | max_count = 1 (lastest session just started)
	}
	return errors.Wrapf(m.db.UpdateTyped(consecutiveUserMiningSessionsSpace, "pk_unnamed_CONSECUTIVE_USER_MINING_SESSIONS_1", key, ops, &[]*consecutiveUserMiningSessions{}),
		"failed to update %v record with the new count consecutive mining sessions for userID:%v", consecutiveUserMiningSessionsSpace, userID)
}
...
func (m *miningEventSourceProcessor) handleMiningSessionStart(userID UserID, lastStartedTS time.Time) (*consecutiveUserMiningSessions, error) {
	...
	// First we need to check if 10 hours passed from the previous session
	// and if not - increment counter
	if time.Since(time.Unix(0, int64(userMiningSessions.LastMiningStartedAt))) >= maxTimeBetweenConsecutiveMiningSessions {
		if err = m.resetConsecutiveMiningSessions(userID, timeStartedNano); err != nil {
		...
		if err = m.updateConsecutiveMiningSessions(userID, timeStartedNano); err != nil {
